...
```
sudo cp run-qgis-3.0ltr-in-docker.sh /usr/local/bin
sudo cp QGIS-3.0LTR.Docker.desktop /usr/share/applications/
...
```
docker build -t kartoza/qgis-desktop:3.0 git://github.com/kartoza/docker-qgis-desktop
```
...
docker run --rm --name="qgis-desktop-3.0ltr" \
	-i -t \
	-v ${HOME}:/home/${USER} \
...
xhost -
```
The above is the content of run-qgis-3.0ltr-in-docker.sh so you can just
```
./run-qgis-3.0ltr-in-docker.sh
```
